...
macro_rules! bot_convar {
    ($engine: expr,$name: expr, $default_text: expr, $flag: expr, $help_text: expr, $token: expr) => {
        $engine
            .register_convar($name.to_string(), "", $help_text, $flag as i32, $token)
            .expect("failed to register convar")
    };
}

pub fn register_required_convars(engine: &EngineData, token: EngineToken) {
    bot_convar!(
        engine,
        "bot_pilot_settings",
        "",
